...
interface (`run_task`) so the crew orchestrator can mix them freely:
  ...
  * **ClaudeAgent** — a real Claude Code session via claude-agent-sdk. It brings
    its *own* tools and permission system (routed through the crew approver), so
    it runs the task as a one-shot Claude Code agent. Imported lazily so the crew
    still works fully when the SDK isn't installed.
...
# 32768 matches the observed loaded context for qwen3-coder and fits in VRAM.
...
COMPACT_AT_TOKENS = int(0.70 * WORKER_NUM_CTX)  # ~22937
...
def _ollama_chat(model: str, messages: list[dict], tools: list[dict] | None,
    ...
    # compaction threshold (see WORKER_NUM_CTX).
...
class ClaudeAgent:
    ...
    def run_task(self, task: str, *, system: str = "", context: str = "",
        ...
        async def _go() -> str:
            ...
            try:  # hand Claude the same MCP servers the local agents use
                from . import mcp_bridge
                servers = mcp_bridge.claude_mcp_config()
                if servers:
            ...
            async with ClaudeSDKClient(options=opts) as client:
                ...
                async for message in client.receive_response():
                    ...
                    if text:
                        chunks.append(text)
                        on_event({"type": "text", "agent": f"claude:{self.model}",
                                  "text": text})
            return "\n".join(chunks).strip()
